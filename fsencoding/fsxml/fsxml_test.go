...
func TestParseXML(t *testing.T) {
	fstest.PrintTestBegin("fsky.pro/fsencoding/fsxml")

	doc, err := LoadString(_xml)
	if err != nil {
	...
	fmt.Printf("xml.<namespace> = %s\n", doc.GetNamespace("xml")) // 获取指定名称的名字空间
	fmt.Printf("a.<namespace> = %s\n", doc.GetNamespace("a"))     // 来自于 <xml:fsky xmlns:a=...>
	...
	// 保存为 xml 文件
	fmt.Println("\n//", strings.Repeat("-", 50), "\n//", "保存为 xml 文件：")
	err = doc.Save("./test.xml") // 保存为非格式化的 xml 文件
	if err != nil {
	...
	doc2 := doc.Clone()                                  // 复制一个文档
	err = doc2.SaveIndent("./test_indent.xml", "\t", LF) // 保存为格式化的 xml 文件
	if err != nil {
	...
	// 读取 xml 文件
	fmt.Println("\n//", strings.Repeat("-", 50), "\n//", "读取 xml 文件：")
	doc3, err := LoadFile("./test_indent.xml")
	if err != nil {
		panic(fmt.Sprintf("load indent xml file fail, error: %s\n", err.Error()))
	}
	fmt.Printf("doc2's children count = %d\n", doc3.Root().ChildCount())
	fstest.PrintTestEnd()
}
