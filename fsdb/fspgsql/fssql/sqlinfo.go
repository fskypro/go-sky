...
func newExecInfo2(err error, sqltx string) *S_ExecInfo {
	return &S_ExecInfo{
		S_SQLInfo: newSQLInfo(err, sqltx),
	}
}
...
// 只针对 SQL 语句，与表无关的查询
// -------------------------------------------------------------------
type S_FetchInfo struct {
	*S_SQLInfo
}

func newFetchInfo(err error, sqltx string) *S_FetchInfo {
	return &S_FetchInfo{
		S_SQLInfo: newSQLInfo(err, sqltx),
	}
}
