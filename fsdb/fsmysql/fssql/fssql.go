...
func explainExp(table *S_Table, exp string, args ...interface{}) (newExp string, inValues []interface{}, err error) {
	inValues = make([]interface{}, 0)
	getArg := func(index int) interface{} {
		...
		arg := args[index]
		if fsutil.IsNil(arg) {
			err = fmt.Errorf("argument %d is not allow to be nil for expression %q", index, exp)
			return nil
		}
		return arg
	}
	...
	// invalue
	repInValue := func(index int, arg interface{}) string {
	...
	// member
	repMember := func(index int, arg interface{}) string {
		switch arg.(type) {
		case *S_Member:
	...
	// table
	repTable := func(index int, arg interface{}) string {
		switch arg.(type) {
		case *S_Table:
			table := arg.(*S_Table)
	...
	replacers := map[string]func(int, interface{}) string{
		"?": repInValue,
		"$": repMember,
		"#": repTable,
	}
