...
func newWriter(w io.Writer, fmtInfo *S_FmtInfo) *s_Writer {
	...
	if writer.isFormat {
		...
		writer.windent = fmtInfo.WholeIndent
...
func (this *s_Writer) _newlineObject(layers int) error {
	if this.isFormat && (this.indentLayers == 0 || this.layers <= this.indentLayers) {
		if err := this._writeString(this.newline); err != nil {
			return err
		}
		return this._writeString(this.windent + strings.Repeat(this.indent, layers))
	}
	return nil
}
...
func _writeOrigin(this *s_Writer, value I_Value) error {
	return this._writeString(fmt.Sprintf("%v", value.FmtString()))
}
