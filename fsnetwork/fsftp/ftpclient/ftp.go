...
type ServerConn struct {
	...
	isActive   bool        // 是否是主动模式
	activeInfo *ActiveInfo // 主动模式信息
}
...
func (c *ServerConn) cmdDataConnFrom(offset uint64, format string, args ...interface{}) (net.Conn, error) {
	...
	if !c.isActive {
		...
		if err = sendCmd(); err != nil {
			_ = conn.Close()
			return nil, fmt.Errorf("send ftp cmd fail, %v", err)
		}
	} else {
		// 主动模式
		if c.activeInfo == nil {
			if err = c.SetToActive(); err != nil {
				return nil, fmt.Errorf("start tcp listening for receive data fail, %v", err)
			}
		}
		// 告知服务器数据传输访问地址
		_, _, err = c.cmd(StatusCommandOK, "PORT "+c.activeInfo.cmdPortArgs)
		if err != nil {
			return nil, fmt.Errorf("set to active ftp fail, %v", err)
		}
		if err = sendCmd(); err != nil {
			return nil, fmt.Errorf("send ftp cmd fail, %v", err)
		}
		...
		// 等待回复连接超时处理
		timeout := c.options.shutTimeout
		if timeout <= 0 { timeout = DefaultDialTimeout }
		timer := time.AfterFunc(timeout, func() {
			c.activeInfo.listener.Close()
			c.activeInfo = nil
		})

		for {
			conn, err = c.activeInfo.listener.Accept()
			if err != nil {
				timer.Stop()
				return nil, fmt.Errorf("accept server data connect fail, %v", err)
			}
			remoteAddr := conn.RemoteAddr().(*net.TCPAddr)
			if remoteAddr.IP.String() == c.host {
				timer.Stop()
				break
			}
			// 排除盗连(程序如果走到这里，说明是盗连)
...
// 修改为主动模式
func (this *ServerConn) SetToActive() error {
	this.isActive = true
	addr := this.netConn.LocalAddr().String()
	host, sport, err := net.SplitHostPort(addr)
	if err != nil {
		return fmt.Errorf("get local host fail, %v", err)
	}
	ip := strings.ReplaceAll(host, ".", ",")
	port, err := strconv.Atoi(sport)
	if err != nil {
		return fmt.Errorf("get local port fail, %v", err)
	}
	port++
	...
	if err != nil {
		return fmt.Errorf("listen %q fail, %v", fmt.Sprintf("%s:%d", host, port), err)
	}
	highPort := port / 256
	lowPort := port % 256
	this.activeInfo = &ActiveInfo{
		listener:    listener,
		cmdPortArgs: fmt.Sprintf("%s,%d,%d", ip, highPort, lowPort),
	}
	return nil
}
